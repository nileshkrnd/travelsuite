...
export type GlobalCodeMasterConfig = {
  /** Prisma model accessor on admin client, e.g. floorType */
  model: keyof typeof prisma;
  /** BigInt primary key field, e.g. floorTypeId */
  idField: string;
  codeField: string;
  nameField: string;
  label: string;
...
function delegate(config: GlobalCodeMasterConfig): GlobalCodeDelegate {
  return prisma[config.model] as unknown as GlobalCodeDelegate;
}
...
export function createGlobalCodeListHandlers(config: GlobalCodeMasterConfig) {
  const descriptionMax = config.descriptionMax ?? 250;
  const createSchema = z.object({
    [config.codeField]: z.string().trim().min(1).max(50),
  ...
  async function GET(request: Request) {
    try {
      ...
      const rows = await delegate(config).findMany({
        where,
        orderBy: [{ displayOrder: "asc" }, { [config.nameField]: "asc" }],
      });
  ...
  async function POST(request: Request) {
    try {
      ...
      const created = await delegate(config).create({
        data: {
          [config.codeField]: String(data[config.codeField]).trim().toUpperCase(),
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") {
        return NextResponse.json({ error: `This ${config.label} code already exists` }, { status: 409 });
      }
      return dbUnavailable(error);
    }
  }
...
export function createGlobalCodeItemHandlers(config: GlobalCodeMasterConfig) {
  const descriptionMax = config.descriptionMax ?? 250;
