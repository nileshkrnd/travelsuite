...
function delegate(config: TenantCodeMasterConfig): TenantCodeDelegate {
  return prisma[config.model] as unknown as TenantCodeDelegate;
}
...
export function createTenantCodeListHandlers(config: TenantCodeMasterConfig) {
  ...
  const createSchema = z.object({
    ...
    [config.nameField]: z.string().trim().min(1).max(nameMax),
  ...
  async function GET(request: Request) {
    try {
      ...
      const rows = await delegate(config).findMany({
        where,
        orderBy: [{ displayOrder: "asc" }, { [config.codeField]: "asc" }],
      });
  ...
  async function POST(request: Request) {
    try {
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") {
        return NextResponse.json({ error: `This ${config.label} code already exists for this company` }, { status: 409 });
      }
      return dbUnavailable(error);
    }
  }
...
export function createTenantCodeItemHandlers(config: TenantCodeMasterConfig) {
  const descriptionMax = config.descriptionMax ?? 250;
  const nameMax = config.nameMax ?? 100;
