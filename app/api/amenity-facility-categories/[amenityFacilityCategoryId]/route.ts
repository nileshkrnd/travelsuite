...
type RouteContext = { params: Promise<{ amenityFacilityCategoryId: string }> };

export async function PUT(request: Request, context: RouteContext) {
  try {
    const { amenityFacilityCategoryId } = await context.params;
    const id = parseId(amenityFacilityCategoryId);
    ...
    const updated = await prisma.amenityFacilityCategory.update({
      where: { amenityFacilityCategoryId: id },
      data: {
...
export async function DELETE(request: Request, context: RouteContext) {
  try {
    ...
    await prisma.amenityFacilityCategory.update({
      where: { amenityFacilityCategoryId: id },
      data: {
        isDeleted: true,
