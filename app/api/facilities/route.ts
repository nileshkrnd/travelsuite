...
/** FacilityID / AmenityFacilityCategoryID are BigInt columns — convert to number before NextResponse.json(). */
function serialize<T extends { facilityId: bigint; amenityFacilityCategoryId: bigint }>(row: T) {
  return {
    ...row,
...
const createSchema = z.object({
  ...
  facilityName: z.string().trim().min(1).max(250),
...
/** Facility master — Restaurant, Gym, Spa, Parking, … grouped under an Amenity/Facility Category. Global. */
export async function GET(request: Request) {
  try {
    ...
    const rows = await prisma.facility.findMany({
      ...
      orderBy: [{ displayOrder: "asc" }, { facilityName: "asc" }],
    });
  ...
  try {
    ...
    const created = await prisma.facility.create({
      data: {
        ...
        facilityCode: data.facilityCode.trim().toUpperCase(),
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") {
      return NextResponse.json({ error: "This facility code already exists" }, { status: 409 });
    }
    return dbUnavailable(error);
  }
}
