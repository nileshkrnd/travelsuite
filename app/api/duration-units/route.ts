...
async function withCompanyName<T extends { companyId: number | null; durationUnitId: bigint }>(rows: T[]) {
  ...
  return rows.map((r) => ({
    ...r,
    durationUnitId: Number(r.durationUnitId),
...
export async function GET(request: Request) {
  try {
    ...
    const where: Prisma.DurationUnitWhereInput = {};
    ...
    const rows = await prisma.durationUnit.findMany({
      where,
      orderBy: [{ displayOrder: "asc" }, { durationUnitCode: "asc" }],
    });
...
export async function POST(request: Request) {
  try {
    ...
    const created = await prisma.durationUnit.create({
      data: {
