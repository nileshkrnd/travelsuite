...
/** Rate Basis list — filter by TenantID / CompanyID. */
export async function GET(request: Request) {
  try {
    ...
    const where: Prisma.RateBasisWhereInput = {};
    ...
    const rows = await prisma.rateBasis.findMany({
      where,
      orderBy: [{ displayOrder: "asc" }, { rateBasisCode: "asc" }],
    });
...
export async function POST(request: Request) {
  try {
    ...
    const created = await prisma.rateBasis.create({
      data: {
