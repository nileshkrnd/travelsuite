...
type RouteContext = { params: Promise<{ rateTypeGroupId: string }> };
...
async function withCompanyName<T extends { companyId: number | null; rateTypeGroupId: bigint }>(row: T) {
  ...
  return { ...row, rateTypeGroupId: Number(row.rateTypeGroupId), companyName: company?.companyName ?? null };
}

export async function GET(_request: Request, context: RouteContext) {
  try {
    const { rateTypeGroupId: raw } = await context.params;
    const id = idSchema.safeParse(raw);
    if (!id.success) return NextResponse.json({ error: "Invalid rate type group id" }, { status: 400 });

    const row = await prisma.rateTypeGroupMaster.findUnique({ where: { rateTypeGroupId: BigInt(id.data) } });
    if (!row) return NextResponse.json({ error: "Rate type group not found" }, { status: 404 });
...
export async function PUT(request: Request, context: RouteContext) {
  try {
    ...
    const updated = await prisma.rateTypeGroupMaster.update({
      where: { rateTypeGroupId: BigInt(id.data) },
      data: {
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      if (error.code === "P2025") return NextResponse.json({ error: "Rate type group not found" }, { status: 404 });
      if (error.code === "P2002") {
...
export async function PATCH(request: Request, context: RouteContext) {
  try {
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2025") {
      return NextResponse.json({ error: "Rate type group not found" }, { status: 404 });
    }
    return dbUnavailable(error);
  }
}
...
export async function DELETE(_request: Request, context: RouteContext) {
  try {
    ...
    await prisma.rateTypeGroupMaster.delete({ where: { rateTypeGroupId: BigInt(id.data) } });
    ...
    if (isForeignKeyRestrictError(error)) {
      return NextResponse.json({ error: "This rate type group has rate types linked to it and cannot be deleted" }, { status: 409 });
    }
    return dbUnavailable(error);
  }
}
