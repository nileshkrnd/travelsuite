...
const createSchema = z.object({
  ...
  taxTypeName: z.string().trim().min(1).max(150),
...
function serialize<T extends { taxTypeId: bigint }>(row: T) {
  return { ...row, taxTypeId: Number(row.taxTypeId) };
}
...
export async function GET(request: Request) {
  try {
    ...
    const where: Prisma.TaxTypeWhereInput = {};
    ...
    const rows = await prisma.taxType.findMany({
      where,
      orderBy: [{ displayOrder: "asc" }, { taxTypeCode: "asc" }],
    });
...
export async function POST(request: Request) {
  try {
    ...
    const created = await prisma.taxType.create({
      data: {
        taxTypeCode: data.taxTypeCode.trim().toUpperCase(),
        taxTypeName: data.taxTypeName.trim(),
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") {
      return NextResponse.json(
        { error: "This tax type code already exists for this company" },
        { status: 409 }
      );
    }
    return dbUnavailable(error);
