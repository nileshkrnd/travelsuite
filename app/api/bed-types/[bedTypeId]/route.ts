...
type RouteContext = { params: Promise<{ bedTypeId: string }> };

export async function PUT(request: Request, context: RouteContext) {
  try {
    const { bedTypeId } = await context.params;
    const id = parseId(bedTypeId);
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      if (error.code === "P2025") return NextResponse.json({ error: "Bed type not found" }, { status: 404 });
      if (error.code === "P2002") {
...
export async function PATCH(request: Request, context: RouteContext) {
  try {
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2025") {
      return NextResponse.json({ error: "Bed type not found" }, { status: 404 });
    }
    return dbUnavailable(error);
  }
}
