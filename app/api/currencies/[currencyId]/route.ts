...
type RouteContext = { params: Promise<{ currencyId: string }> };

export async function GET(_request: Request, context: RouteContext) {
  try {
    const { currencyId } = await context.params;
    const id = idSchema.safeParse(currencyId);
    if (!id.success) return NextResponse.json({ error: "Invalid currency id" }, { status: 400 });

    const row = await prisma.currency.findUnique({ where: { currencyId: id.data } });
    if (!row) return NextResponse.json({ error: "Currency not found" }, { status: 404 });
...
export async function PUT(request: Request, context: RouteContext) {
  try {
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      if (error.code === "P2025") return NextResponse.json({ error: "Currency not found" }, { status: 404 });
      if (error.code === "P2002") {
