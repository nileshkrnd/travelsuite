...
async function withCompanyName<T extends { companyId: number | null; bookingModelId: bigint }>(rows: T[]) {
  ...
  return rows.map((r) => ({
    ...r,
    bookingModelId: Number(r.bookingModelId),
...
export async function GET(request: Request) {
  try {
    ...
    const where: Prisma.BookingModelWhereInput = {};
    ...
    const rows = await prisma.bookingModel.findMany({
      where,
      orderBy: [{ displayOrder: "asc" }, { bookingModelCode: "asc" }],
    });
...
export async function POST(request: Request) {
  try {
    ...
    const created = await prisma.bookingModel.create({
      data: {
