...
type RouteContext = { params: Promise<{ commonStatusTypeId: string }> };
...
async function withCompanyName<T extends { companyId: number | null; commonStatusTypeId: bigint }>(row: T) {
  ...
  return { ...row, commonStatusTypeId: Number(row.commonStatusTypeId), companyName: company?.companyName ?? null };
}

export async function GET(_request: Request, context: RouteContext) {
  try {
    const { commonStatusTypeId: raw } = await context.params;
    const id = idSchema.safeParse(raw);
    if (!id.success) return NextResponse.json({ error: "Invalid status type id" }, { status: 400 });

    const row = await prisma.commonStatusType.findUnique({ where: { commonStatusTypeId: BigInt(id.data) } });
    if (!row) return NextResponse.json({ error: "Status type not found" }, { status: 404 });
...
export async function PUT(request: Request, context: RouteContext) {
  try {
    ...
    const updated = await prisma.commonStatusType.update({
      where: { commonStatusTypeId: BigInt(id.data) },
      data: {
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      if (error.code === "P2025") return NextResponse.json({ error: "Status type not found" }, { status: 404 });
      if (error.code === "P2002") {
...
export async function PATCH(request: Request, context: RouteContext) {
  try {
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2025") {
      return NextResponse.json({ error: "Status type not found" }, { status: 404 });
    }
    return dbUnavailable(error);
  }
}
...
export async function DELETE(_request: Request, context: RouteContext) {
  try {
    ...
    await prisma.commonStatusType.delete({ where: { commonStatusTypeId: BigInt(id.data) } });
    ...
    if (isForeignKeyRestrictError(error)) {
      return NextResponse.json({ error: "This status type has statuses linked to it and cannot be deleted" }, { status: 409 });
    }
    return dbUnavailable(error);
  }
}
