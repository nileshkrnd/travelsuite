...
type RouteContext = { params: Promise<{ commonStatusId: string }> };
...
async function withCompanyName<T extends { companyId: number | null; commonStatusId: bigint; commonStatusTypeId: bigint }>(row: T) {
  ...
  return { ...row, commonStatusId: Number(row.commonStatusId), commonStatusTypeId: Number(row.commonStatusTypeId), companyName: company?.companyName ?? null };
}

export async function GET(_request: Request, context: RouteContext) {
  try {
    const { commonStatusId: raw } = await context.params;
    const id = idSchema.safeParse(raw);
    if (!id.success) return NextResponse.json({ error: "Invalid status id" }, { status: 400 });

    const row = await prisma.commonStatus.findUnique({ where: { commonStatusId: BigInt(id.data) }, include: rowInclude });
...
export async function PUT(request: Request, context: RouteContext) {
  try {
    ...
    const updated = await prisma.commonStatus.update({
      where: { commonStatusId: BigInt(id.data) },
      data: {
...
export async function DELETE(_request: Request, context: RouteContext) {
  try {
    ...
    await prisma.commonStatus.delete({ where: { commonStatusId: BigInt(id.data) } });
    ...
    if (isForeignKeyRestrictError(error)) {
      return NextResponse.json({ error: "This status is in use and cannot be deleted" }, { status: 409 });
    }
    return dbUnavailable(error);
  }
}
