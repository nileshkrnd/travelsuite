...
const createSchema = propertyContractMarketRuleWriteSchema.and(
...
export async function GET(request: Request) {
  try {
    ...
    const where: Prisma.PropertyContractMarketRuleWhereInput = {};
    ...
    const rows = await prisma.propertyContractMarketRule.findMany({
      where,
      include: propertyContractMarketRuleInclude,
      orderBy: [{ createdDtTm: "desc" }],
    });
    return NextResponse.json(rows.map(serializePropertyContractMarketRuleRow));
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    const created = await createPropertyContractMarketRule(data);
    return NextResponse.json(serializePropertyContractMarketRuleRow(created), { status: 201 });
  } catch (error) {
    return dbUnavailable(error);
  }
}
