...
type RouteContext = { params: Promise<{ propertyTypeId: string }> };
...
export async function GET(_request: Request, context: RouteContext) {
  try {
    const { propertyTypeId: raw } = await context.params;
    const id = idSchema.safeParse(raw);
    if (!id.success) return NextResponse.json({ error: "Invalid property type id" }, { status: 400 });

    const row = await prisma.propertyType.findUnique({ where: { propertyTypeId: id.data } });
    if (!row) return NextResponse.json({ error: "Property type not found" }, { status: 404 });
...
export async function DELETE(_request: Request, context: RouteContext) {
  try {
    ...
    await prisma.propertyType.delete({ where: { propertyTypeId: id.data } });
