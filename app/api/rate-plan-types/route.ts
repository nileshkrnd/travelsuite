...
/** Rate Plan Type list — filter by TenantID / CompanyID. */
export async function GET(request: Request) {
  try {
    ...
    const where: Prisma.RatePlanTypeWhereInput = {};
    ...
    const rows = await prisma.ratePlanType.findMany({
      where,
      orderBy: [{ displayOrder: "asc" }, { ratePlanTypeCode: "asc" }],
    });
...
export async function POST(request: Request) {
  try {
    ...
    const created = await prisma.ratePlanType.create({
      data: {
    ...
    const [withName] = await withCompanyName([serialize(created)]);
