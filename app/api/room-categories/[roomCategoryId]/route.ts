...
type RouteContext = { params: Promise<{ roomCategoryId: string }> };

export async function PUT(request: Request, context: RouteContext) {
  try {
    const { roomCategoryId } = await context.params;
    const id = parseId(roomCategoryId);
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      if (error.code === "P2025") return NextResponse.json({ error: "Room category not found" }, { status: 404 });
      if (error.code === "P2002") {
