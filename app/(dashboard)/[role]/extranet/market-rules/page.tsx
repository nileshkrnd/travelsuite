...
import { PropertyContractMarketRulesList } from "@/components/masters/PropertyContractMarketRulesList";
...
function PropertyMarketRulesPage({ roleDef }: { roleDef: RoleDef }) {
  ...
  return (
    <div className="space-y-6 p-6">
      <PageHeader
        title="Market Rules"
        description="Include/exclude source markets — by country, region, city, or market group — across supplier contracts for the selected property."
      />
      ...
      <ExtranetPropertyScope
        ...
        emptyHeading="Select a property to view market rules"
        emptyDescription="Choose the property whose market rules you want to review or manage."
      >
        {propertyId && propertyId > 0 ? (
          <PropertyContractMarketRulesList
            tenantId={tenantKey}
            propertyId={propertyId}
            canEdit={canEdit}
            canCreate={canCreate}
...
export default function ExtranetMarketRulesPage() {
  return (
    <AccessGate module="marketRules">
      {(roleDef) => (
        <Suspense fallback={<div className="p-6 text-sm text-muted-foreground">Loading…</div>}>
          <PropertyMarketRulesPage roleDef={roleDef} />
        </Suspense>
      )}
    </AccessGate>
  );
