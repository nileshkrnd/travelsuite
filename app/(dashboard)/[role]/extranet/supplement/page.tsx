...
import { PropertyContractSupplementsList } from "@/components/masters/PropertyContractSupplementsList";
...
function PropertySupplementsPage({ roleDef }: { roleDef: RoleDef }) {
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <ExtranetPropertyScope
        ...
        emptyHeading="Select a property to view supplements"
        emptyDescription="Choose the property whose contract supplements you want to review or manage."
      >
        {propertyId && propertyId > 0 ? (
          <PropertyContractSupplementsList
            tenantId={tenantKey}
            propertyId={propertyId}
            canEdit={canEdit}
            canCreate={canCreate}
...
export default function ExtranetSupplementPage() {
  return (
    <AccessGate module="supplement">
      {(roleDef) => (
        <Suspense fallback={<div className="p-6 text-sm text-muted-foreground">Loading…</div>}>
          <PropertySupplementsPage roleDef={roleDef} />
        </Suspense>
      )}
    </AccessGate>
  );
