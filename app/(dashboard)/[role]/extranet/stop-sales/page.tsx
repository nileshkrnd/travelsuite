...
import { PropertyContractStopSalesList } from "@/components/masters/PropertyContractStopSalesList";
...
function PropertyStopSalesPage({ roleDef }: { roleDef: RoleDef }) {
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <ExtranetPropertyScope
        ...
        emptyHeading="Select a property to view stop sales"
        emptyDescription="Choose the property whose stop-sale rules you want to review or manage."
      >
        {propertyId && propertyId > 0 ? (
          <PropertyContractStopSalesList
            tenantId={tenantKey}
            propertyId={propertyId}
            canEdit={canEdit}
            canCreate={canCreate}
...
export default function ExtranetStopSalesPage() {
  return (
    <AccessGate module="stopSales">
      {(roleDef) => (
        <Suspense fallback={<div className="p-6 text-sm text-muted-foreground">Loading…</div>}>
          <PropertyStopSalesPage roleDef={roleDef} />
        </Suspense>
      )}
    </AccessGate>
  );
