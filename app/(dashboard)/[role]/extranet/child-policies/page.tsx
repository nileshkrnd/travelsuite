...
import { PropertyContractChildPoliciesList } from "@/components/masters/PropertyContractChildPoliciesList";
...
function PropertyChildPoliciesPage({ roleDef }: { roleDef: RoleDef }) {
  ...
  return (
    <div className="space-y-6 p-6">
      <PageHeader
        ...
        description="Maximum children, occupancy rules, and age-band pricing across supplier contracts for the selected property."
      />
      ...
      <ExtranetPropertyScope
        ...
        emptyHeading="Select a property to view child policies"
        emptyDescription="Choose the property whose child policies you want to review or manage."
      >
        {propertyId && propertyId > 0 ? (
          <PropertyContractChildPoliciesList
            tenantId={tenantKey}
            propertyId={propertyId}
            canEdit={canEdit}
            canCreate={canCreate}
...
export default function ExtranetChildPoliciesPage() {
  return (
    <AccessGate module="childPolicies">
      {(roleDef) => (
        <Suspense fallback={<div className="p-6 text-sm text-muted-foreground">Loading…</div>}>
          <PropertyChildPoliciesPage roleDef={roleDef} />
        </Suspense>
      )}
    </AccessGate>
  );
