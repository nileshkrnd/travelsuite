...
import { PropertyContractRatesList } from "@/components/masters/PropertyContractRatesList";
...
function PropertyRatesPage({ roleDef }: { roleDef: RoleDef }) {
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <ExtranetPropertyScope
        ...
        emptyHeading="Select a property to view rates"
        emptyDescription="Choose the property whose contract rates you want to review or enter."
      >
        {propertyId && propertyId > 0 ? (
          <PropertyContractRatesList
            tenantId={tenantKey}
            propertyId={propertyId}
            canEdit={canEdit}
            canCreate={canCreate}
...
export default function ExtranetRatesPage() {
  return (
    <AccessGate module="rates">
      {(roleDef) => (
        <Suspense fallback={<div className="p-6 text-sm text-muted-foreground">Loading…</div>}>
          <PropertyRatesPage roleDef={roleDef} />
        </Suspense>
      )}
    </AccessGate>
  );
