...
import { Plus, Clock3, Package, Eye, Pencil, Power, PowerOff, Trash2, X, Search, Loader2 } from "lucide-react";
...
function ScheduleList({ roleDef }: { roleDef: RoleDef }) {
  ...
  async function loadServiceTypes() {
    if (scopeTenantId <= 0) {
      ...
      setLoadError(platformMode ? "Select a tenant workspace to manage schedules." : "Missing tenant scope.");
      return;
    }
  ...
  useEffect(() => {
    ...
    Promise.all([
      listServiceProductAvailabilities({ serviceProductId: productFilter, activeOnly: true }),
  ...
  return (
    <div className="space-y-6 p-6">
      <PageHeader
        title="Service Product Schedule"
      ...
      {!loadingTypes && serviceTypes.length > 0 && (
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          ) : productFilter && availabilities.length === 0 ? (
            <p className="text-sm text-muted-foreground">No availability windows yet — create one first.</p>
          ) : (
                    ...
                    <TableCell className="px-2 py-1.5 text-right">
                      <div className="flex items-center justify-end gap-0.5">
                        <Tooltip>
                          <TooltipTrigger
                            render={
                              <Button variant="ghost" size="icon-sm" aria-label="View" onClick={() => { setTarget(row); setPanelMode("view"); }} />
                            }
                          >
                        ...
                        {canEdit && (
                          <>
                            <Tooltip>
                              <TooltipTrigger
                                render={
                                  <Button variant="ghost" size="icon-sm" aria-label="Edit" onClick={() => { setTarget(row); setPanelMode("edit"); }} />
                                }
                              >
                            ...
                            <Tooltip>
                              <TooltipTrigger
                                render={
                                  <Button variant="ghost" size="icon-sm" aria-label={row.isActive ? "Deactivate" : "Activate"} onClick={() => void toggleActive(row)} />
                                }
                              >
                        ...
                        {canDelete && (
                          <Tooltip>
                            <TooltipTrigger
                              render={
                                <Button variant="ghost" size="icon-sm" aria-label="Delete" onClick={() => void removeRow(row)} />
                              }
                            >
      ...
      {!selectedAvailability && !loadingTypes && serviceTypes.length > 0 && (
        <EmptyState icon={Package} tone="muted" heading="Select an availability window" description="Choose a service type, product, and availability window above to manage its schedules." size="compact" />
      )}
    </div>
  );
}

export default function ServiceProductScheduleMasterPage() {
  return <AccessGate module="serviceProductSchedule">{(roleDef) => <ScheduleList roleDef={roleDef} />}</AccessGate>;
}
