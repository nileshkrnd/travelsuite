...
import { Plus, Building2, Package, Eye, Pencil, Power, PowerOff, Trash2, X, Search, Loader2, Star } from "lucide-react";
...
function SupplierLocationList({ roleDef }: { roleDef: RoleDef }) {
  ...
  async function loadServiceTypes() {
    if (scopeTenantId <= 0) {
      ...
      setLoadError(platformMode ? "Select a tenant workspace to manage supplier locations." : "Missing tenant scope.");
      return;
    }
  ...
  return (
    <div className="space-y-6 p-6">
      <PageHeader
        title="Service Product Supplier Location"
        description="A supplier's own locations for a product — may reference a canonical product location, or stand alone."
        actions={
      ...
      {!selectedSupplierLink && !loadingTypes && serviceTypes.length > 0 && (
        <EmptyState icon={Package} tone="muted" heading="Select a supplier" description="Choose a service type, product, and supplier above to manage its locations." size="compact" />
      )}
    </div>
  );
}

export default function ServiceProductSupplierLocationMasterPage() {
  return <AccessGate module="serviceProductSupplierLocation">{(roleDef) => <SupplierLocationList roleDef={roleDef} />}</AccessGate>;
}
