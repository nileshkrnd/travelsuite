...
import { Plus, Coins, MoreHorizontal, X, Search, Loader2 } from "lucide-react";
      ...
      <Card>
        {!loading && rows.length === 0 && scopeTenantId > 0 ? (
          ...
          <Table>
            <TableHeader>
              <TableRow>
                <SortableTableHead sortKey="pricingModelCode" activeKey={sortKey} direction={sortDirection} onSort={toggleSort}>
                  Code
                </SortableTableHead>
                <SortableTableHead sortKey="pricingModelName" activeKey={sortKey} direction={sortDirection} onSort={toggleSort}>
                  Name
                </SortableTableHead>
            ...
            <TableBody>
              {visible.map((row) => (
                <TableRow key={row.pricingModelId}>
                  <TableCell className="font-mono text-xs font-medium">{row.pricingModelCode}</TableCell>
                  <TableCell className="font-medium">{row.pricingModelName}</TableCell>
