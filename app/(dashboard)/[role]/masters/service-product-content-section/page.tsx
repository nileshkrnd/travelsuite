...
function View({ roleDef }: { roleDef: RoleDef }) {
  ...
  useEffect(() => {
    if (scopeTenantId <= 0) {
      ...
      setLoadError(platformMode ? "Select a tenant workspace to manage content sections." : "Missing tenant scope.");
      return;
    }
  ...
  return (
    <div className="space-y-6 p-6">
      <PageHeader
        title="Service Product Content Sections"
        description="What to Expect, Additional Info, Know Before You Go, Important Information, What to Bring — for a specific product."
      />
      ...
      {selectedProduct && <ProductContentSectionTab product={selectedProduct} roleDef={roleDef} />}
    </div>
  );
}

export default function ServiceProductContentSectionMasterPage() {
  return <AccessGate module="serviceProductContentSection">{(roleDef) => <View roleDef={roleDef} />}</AccessGate>;
}
