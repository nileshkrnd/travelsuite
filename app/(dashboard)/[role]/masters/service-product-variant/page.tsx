...
import { Plus, Layers, Package, Eye, Pencil, Power, PowerOff, Trash2, X, Search, Loader2, Star } from "lucide-react";
...
import type { CommonStatus, RoleDef, ServiceProduct, ServiceProductOption, ServiceProductVariant, ServiceType } from "@/types";
...
function VariantList({ roleDef }: { roleDef: RoleDef }) {
  ...
  async function loadServiceTypes() {
    if (scopeTenantId <= 0) {
      ...
      setLoadError(platformMode ? "Select a tenant workspace to manage variants." : "Missing tenant scope.");
      return;
    }
  ...
  useEffect(() => {
    ...
    listServiceProductOptions({ serviceProductId: productFilter, activeOnly: true })
    ...
    <div className="space-y-6 p-6">
      ...
      {!loadingTypes && serviceTypes.length > 0 && (
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:flex-wrap">
          ...
          ) : productFilter && options.length === 0 ? (
            <p className="text-sm text-muted-foreground">No options under this product yet.</p>
          ) : options.length > 0 ? (
      ...
      {!selectedOption && !loadingTypes && serviceTypes.length > 0 && (
        <EmptyState icon={Package} tone="muted" heading="Select an option" description="Choose a service type, product, and option above to manage its variants." size="compact" />
      )}
    </div>
  );
}

export default function ServiceProductVariantMasterPage() {
  return <AccessGate module="serviceProductVariant">{(roleDef) => <VariantList roleDef={roleDef} />}</AccessGate>;
}
