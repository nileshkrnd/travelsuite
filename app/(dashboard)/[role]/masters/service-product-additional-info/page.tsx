...
function View({ roleDef }: { roleDef: RoleDef }) {
  ...
  useEffect(() => {
    if (scopeTenantId <= 0) {
      ...
      setLoadError(platformMode ? "Select a tenant workspace to manage additional info." : "Missing tenant scope.");
      return;
    }
  ...
  return (
    <div className="space-y-6 p-6">
      <PageHeader
        title="Service Product Additional Info"
        description="Structured accessibility/limit facts for a specific product."
      />
      ...
      {selectedProduct && <ProductAdditionalInfoTab product={selectedProduct} roleDef={roleDef} />}
    </div>
  );
}

export default function ServiceProductAdditionalInfoMasterPage() {
  return <AccessGate module="serviceProductAdditionalInfo">{(roleDef) => <View roleDef={roleDef} />}</AccessGate>;
}
