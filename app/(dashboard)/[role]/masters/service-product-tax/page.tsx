...
import { Plus, Percent, Package, Eye, Pencil, Power, PowerOff, Trash2, X, Loader2 } from "lucide-react";
...
function TaxList({ roleDef }: { roleDef: RoleDef }) {
  ...
  useEffect(() => {
    Promise.all([listTaxCalculationTypes({ activeOnly: true }), listTaxApplicationBasis({ activeOnly: true })])
      .then(([calc, basis]) => {
        setCalcTypes(calc);
  ...
  async function loadServiceTypes() {
    if (scopeTenantId <= 0) {
      ...
      setLoadError(platformMode ? "Select a tenant workspace to manage taxes." : "Missing tenant scope.");
      return;
    }
  ...
  useEffect(() => {
    if (scopeTenantId <= 0) return;
    listTaxes({ tenantId: scopeTenantId, activeOnly: true })
      .then(setTaxes)
  ...
  async function refreshRows() {
    ...
    try {
      const [rowsResult, supplierRows, optionRows] = await Promise.all([
        listServiceProductTaxes({ serviceProductId: productFilter }),
  ...
  return (
    <div className="space-y-6 p-6">
      <PageHeader
        title="Service Product Tax"
        description="Taxes applied to a Service Product — optionally scoped to a supplier, option, or variant."
        actions={
          canCreate && panelMode === "closed" && selectedProduct && taxes.length > 0 && calcTypes.length > 0 && applicationBasisRows.length > 0 ? (
      ...
      {selectedProduct && (
        <Card>
          {loadingRows ? (
            ...
            <EmptyState icon={Percent} tone="primary" heading="No taxes yet" description={`Add a tax under ${selectedProduct.serviceProductName}.`} size="compact" />
          ) : visible.length === 0 ? (
      ...
      {!selectedProduct && !loadingTypes && serviceTypes.length > 0 && (
        <EmptyState icon={Package} tone="muted" heading="Select a product" description="Choose a service type and product above to manage its taxes." size="compact" />
      )}
    </div>
  );
}

export default function ServiceProductTaxMasterPage() {
  return <AccessGate module="serviceProductTax">{(roleDef) => <TaxList roleDef={roleDef} />}</AccessGate>;
}
