...
function View({ roleDef }: { roleDef: RoleDef }) {
  ...
  useEffect(() => {
    if (scopeTenantId <= 0) {
      ...
      setLoadError(platformMode ? "Select a tenant workspace to manage requirements." : "Missing tenant scope.");
      return;
    }
  ...
  return (
    <div className="space-y-6 p-6">
      <PageHeader
        title="Service Product Requirements"
        description="Traveler requirements — passport, visa, age, waiver — for a specific product."
      />
      ...
      {selectedProduct && <ProductRequirementTab product={selectedProduct} roleDef={roleDef} />}
    </div>
  );
}

export default function ServiceProductRequirementMasterPage() {
  return <AccessGate module="serviceProductRequirement">{(roleDef) => <View roleDef={roleDef} />}</AccessGate>;
}
