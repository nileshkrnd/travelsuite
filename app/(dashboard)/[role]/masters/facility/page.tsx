...
import { Plus, Wrench, MoreHorizontal, X, Search, CheckCircle2 } from "lucide-react";
...
import type { AmenityFacilityCategory, Facility, RoleDef } from "@/types";

type PanelMode = "closed" | "create" | "edit" | "view";
type SortKey = "name" | "code" | "displayOrder";
type StatusFilter = "all" | "active" | "inactive";

function useFacilitySchema(rows: Facility[], currentId?: string) {
  return z.object({
    ...
    code: z
      ...
      .refine(
        ...
        "This facility code already exists"
      ),
    name: z.string().trim().min(1, "Facility name is required").max(250, "Must be 250 characters or fewer"),
...
type FormValues = z.infer<ReturnType<typeof useFacilitySchema>>;
...
function FacilityPanel({
  ...
  const schema = useFacilitySchema(rows, row?.id);
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      if (mode === "edit" && row) {
        const saved = await updateFacility(row.facilityKey, {
      ...
      toast.error(error instanceof FacilitiesApiError ? error.message : "Could not save facility");
    }
  }
    ...
    <Card className="p-6">
      <div className="mb-4 flex items-start justify-between gap-4">
        <h2 className="text-base font-semibold">
          {mode === "create" ? "Add facility" : mode === "edit" ? "Edit facility" : "Facility details"}
        </h2>
...
function FacilityList({ roleDef }: { roleDef: RoleDef }) {
  ...
  const [target, setTarget] = useState<Facility | undefined>();
  const [search, setSearch] = useState("");
  const [statusFilter, setStatusFilter] = useState<StatusFilter>("all");
  const [categoryFilter, setCategoryFilter] = useState<string>("all");
  const [sortKey, setSortKey] = useState<SortKey | null>(null);
  const [sortDirection, setSortDirection] = useState<SortDirection>("asc");
  const canEdit = can(roleDef, "facility", "edit");
  const canCreate = can(roleDef, "facility", "create");
  const canDelete = can(roleDef, "facility", "delete");
  const actorKey = user ? (users.find((u) => u.id === user.id)?.userKey ?? user.userKey ?? 0) : 0;

  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    Promise.all([listFacilities(), listAmenityFacilityCategories({ activeOnly: true })])
      .then(([facilityRows, categoryRows]) => {
        if (cancelled) return;
        setRows(facilityRows);
        setCategories(categoryRows);
        setLoading(false);
      })
      .catch((err) => {
        if (cancelled) return;
        setLoadError(err instanceof FacilitiesApiError ? err.message : "Failed to load");
        setLoading(false);
      });
    return () => {
      cancelled = true;
  ...
  function upsertLocal(row: Facility) {
    setRows((prev) => {
  ...
  async function toggleActive(row: Facility) {
    if (!actorKey) {
      toast.error("Missing user key — sign in again.");
      return;
    }
    try {
      const saved = await setFacilityActive(row.facilityKey, !row.isActive, actorKey);
      upsertLocal(saved);
      toast.success(saved.isActive ? "Activated" : "Deactivated");
    } catch (error) {
      toast.error(error instanceof FacilitiesApiError ? error.message : "Could not update status");
    }
  }

  async function removeRow(row: Facility) {
    if (!actorKey) {
      toast.error("Missing user key — sign in again.");
      return;
    }
    try {
      await deleteFacility(row.facilityKey, actorKey);
      setRows((prev) => prev.filter((r) => r.id !== row.id));
      toast.success("Facility deleted");
    } catch (error) {
      toast.error(error instanceof FacilitiesApiError ? error.message : "Could not delete");
    }
  }
  ...
  return (
    <div className="space-y-6 p-6">
      <PageHeader
        title="Facility"
        description="Global lookup of property-level facilities (Restaurant, Gym, Spa, Parking, …), grouped by category."
        actions={
      ...
      {!loading && categories.length === 0 && (
        <p className="text-sm text-muted-foreground">
          Create an Amenity/Facility Category first before adding facilities.
        </p>
      )}
      ...
      <Card>
        {!loading && rows.length === 0 ? (
          <EmptyState
            ...
            description="Add your first entry (e.g. Restaurant, Gym)."
            size="compact"
          />
...
export default function FacilityMasterPage() {
  return <AccessGate module="facility">{(roleDef) => <FacilityList roleDef={roleDef} />}</AccessGate>;
}
