...
import { Plus, MapPin, Eye, Pencil, Power, PowerOff, Trash2, X, Search, Loader2, Star } from "lucide-react";
...
export function ProductLocationTab({ product, roleDef }: { product: ServiceProduct; roleDef: RoleDef }) {
  ...
  async function refreshRows() {
    setLoading(true);
    try {
      const rowsResult = await listServiceProductLocations({ serviceProductId: product.serviceProductId });
      setRows(rowsResult);
    } catch (error) {
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title="Location"
        description="Pickup points, meeting points, destinations, venues, … tied to this product."
        actions={
          canCreate && panelMode === "closed" && locationTypes.length > 0 && statuses.length > 0 ? (
      ...
      {panelMode !== "closed" && (
        <LocationPanel mode={panelMode} row={target} rows={rows} product={product} locationTypes={locationTypes} countries={countries} regions={regions} statuses={statuses} userKey={userKey} onSaved={refreshRows} onClose={() => { setPanelMode("closed"); setTarget(undefined); }} />
      )}
      ...
      <Card>
        {loading ? (
          ...
          <EmptyState icon={MapPin} tone="primary" heading="No locations yet" description="Add a location for this product." size="compact" />
        ) : visible.length === 0 ? (
