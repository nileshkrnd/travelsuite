...
import { Plus, DollarSign, Building2, Eye, Pencil, Power, PowerOff, Trash2, X, Search, Loader2 } from "lucide-react";
...
export function ProductRateTab({ product, roleDef }: { product: ServiceProduct; roleDef: RoleDef }) {
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all([
      listRateTypes({ tenantId: product.tenantId, activeOnly: true }),
      listDayOfWeeks({ activeOnly: true }),
      listCommonStatusTypes({ tenantId: product.tenantId, activeOnly: true }),
    ]).then(async ([rateTypeRows, dayRows, statusTypeRows]) => {
      if (cancelled) return;
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      listServiceProductRates({ serviceProductId: product.serviceProductId }),
    ])
  ...
  return (
    <div className="space-y-4">
      ...
      {panelMode !== "closed" && selectedSupplierLink && (
        <RatePanel mode={panelMode} row={target} rows={rows} product={product} supplierLink={selectedSupplierLink} rateTypes={rateTypes} statuses={statuses} days={days} userKey={userKey} onSaved={refreshRows} onClose={() => { setPanelMode("closed"); setTarget(undefined); }} />
      )}
