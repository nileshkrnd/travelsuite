...
import { Plus, Route, Eye, Pencil, Power, PowerOff, Trash2, X, Search, Loader2, Moon, Star } from "lucide-react";
...
import type { CommonStatus, DurationUnit, RoleDef, ServiceProduct, ServiceProductItinerary, ServiceProductLocation } from "@/types";
    ...
    <Card className="p-6">
      ...
      <form onSubmit={handleSubmit((values) => submit(values, false))} className="grid grid-cols-2 gap-3 sm:grid-cols-4" noValidate>
        ...
        <div className="space-y-1 sm:col-span-2">
          ...
          <Input id="title" disabled={isReadOnly} placeholder="e.g. Visit Old Town Souq" aria-invalid={!!errors.title} {...register("title")} />
...
export function ProductItineraryTab({ product, roleDef }: { product: ServiceProduct; roleDef: RoleDef }) {
  ...
  useEffect(() => {
    ...
    ]).then(async ([durationUnitRows, locationRows, statusTypeRows]) => {
      if (cancelled) return;
  ...
  }, [product.tenantId, product.serviceProductId]);

  async function refreshRows() {
    setLoading(true);
    try {
      const rowsResult = await listServiceProductItineraries({ serviceProductId: product.serviceProductId });
      setRows(rowsResult);
    } catch (error) {
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title="Itinerary"
        description="Day-by-day stops for this product — sequence, timing, location, and highlights."
        actions={
      ...
      {panelMode !== "closed" && (
        <ItineraryPanel mode={panelMode} row={target} rows={rows} product={product} durationUnits={durationUnits} locations={locations} statuses={statuses} userKey={userKey} onSaved={refreshRows} onClose={() => { setPanelMode("closed"); setTarget(undefined); }} />
      )}
      ...
      <Card>
        {loading ? (
          ...
          <EmptyState icon={Route} tone="primary" heading="No itinerary stops yet" description="Add a stop for this product." size="compact" />
        ) : visible.length === 0 ? (
